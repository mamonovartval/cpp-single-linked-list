...
private:
	...
	void Assign(InputIterator from, InputIterator to) {
		...
		while (from != to) {
			...
			//записываю в node_ptr указатель на новый объект
			*node_ptr = item;
			//записываю в указатель на указатель **node_ptr ссылку на next_node созданного объекта
